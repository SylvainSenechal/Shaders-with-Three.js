...
function initGUI(){
	...
	var controller = gui.add(colorS, 'color')
...
function createScene(){
	...
	// COMPOSER START
	var light = new THREE.PointLight( 0xff00, 1.5, 4000 );
	...
	var effectRays = new THREE.ShaderPass( THREE.ShaderGodRays )
	effectRays.renderToScreen = true
	composer.addPass( effectRays )
	...
	// // sobel
	// var eff = THREE.LuminosityShader
	// var effectGrayScale = new THREE.ShaderPass( eff );
	// composer.addPass( effectGrayScale );
	// var shaderSepia = THREE.SobelOperatorShader;
	// var effectSepia = new THREE.ShaderPass( shaderSepia );
	// effectSepia.uniforms.resolution.value.x = window.innerWidth;
	// effectSepia.uniforms.resolution.value.y = window.innerHeight;
	// effectSepia.renderToScreen = true;
	// composer.addPass(effectSepia);

	// composer = new THREE.EffectComposer(renderer);
	//  var sunRenderModel = new THREE.RenderPass(scene, camera);
	//  var effectBloom = new THREE.BloomPass(1, 25, 1);
...
function createLights(){
	...
	shadowLight.position.set(0, 2000, 0);
...
/*	var urls = [
    ...
    this.skybox = new THREE.Mesh( new THREE.CubeGeometry( 10000, 10000, 10000 ), matBox );
